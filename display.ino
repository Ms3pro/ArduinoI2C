...
unsigned int frameColor = shouldBlinkFrame ? (millis() % 10 < 5 ? ST7735_WHITE : ST7735_RED) : determineFrameColor();  
...
for (int i = 0; i < 6; i++) {
    if (egtValues[i] > 800) {
        barBlinking[i] = true;
        if (millis() - lastBlinkTime[i] > 10) { // Переключение каждые 1500 мс
...
void readEGTValues() {
  ...
  analogWrite(EGT1_LED, egt1 > 800 ? HIGH : LOW);
  analogWrite(EGT2_LED, egt2 > 800 ? HIGH : LOW);
  analogWrite(EGT3_LED, egt3 > 800 ? HIGH : LOW);
  analogWrite(EGT4_LED, egt4 > 800 ? HIGH : LOW);
  analogWrite(EGT5_LED, egt5 > 800 ? HIGH : LOW);
  analogWrite(EGT6_LED, egt6 > 800 ? HIGH : LOW);
...
void initialise_display()
{
  ...
  tft.fillScreen(WHITE);
    ...
    tft.drawRGBBitmap (0,15, epd_bitmap_MS3PRO_1, 160, 80);//MS3PRO
